...
def retrieve_entries():
    ...
    eos_wb = openpyxl.load_workbook('M:\Maintenance EOS.xlsx')
    eos_ws = eos_wb['Archive']
    last_row = eos_ws.max_row + 1
    eos_ws['A' + str(last_row)] = user_date
    eos_ws['B' + str(last_row)] = user_shift
    eos_ws['C' + str(last_row)] = user_workstation
    eos_ws['D' + str(last_row)] = user_specific_area
    eos_ws['E' + str(last_row)] = user_minutes_spent
    eos_ws['F' + str(last_row)] = user_comments
    eos_wb.save('M:\Maintenance EOS.xlsx')
    eos_wb.close()
...
date_entry_var.set(datetime.now().strftime('%m/%d/%Y'))
